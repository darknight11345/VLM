...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for idx, j in enumerate(range(0, len(sources), 2)):
            ...
            gpt_output_text = f"{gpt_response['content']}"
            
            if idx == 0:
                inputs = processor(text=user_input_text, images=images, return_tensors='pt')
        ...
        all_input_ids.append(torch.tensor([2]))  # eos token id
        all_labels.append(torch.tensor([2]))  # eos token id
        ...
        pixel_values = all_pixel_values[0] if all_pixel_values is not None else None
