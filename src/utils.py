...
import torch
from transformers import BitsAndBytesConfig, LlavaForConditionalGeneration, AutoProcessor, AutoConfig
import warnings
import os
import json
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, 
    ...
    if 'lora' in model_name.lower() and model_base is not None:
        ...
        print('Loading Pixtral from base model...')

        model = LlavaForConditionalGeneration.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, trust_remote_code=True, **kwargs)
        ...
        processor = AutoProcessor.from_pretrained(model_base, trust_remote_code=True)
        model = LlavaForConditionalGeneration.from_pretrained(model_path, low_cpu_mem_usage=True, trust_remote_code=True, **kwargs)
